...
protected:
    ...
    const std::vector<int> signalRoles = {
        DbcFile::Role_Unit, DbcFile::Role_StartBit, DbcFile::Role_BitLength,
        DbcFile::Role_Factor, DbcFile::Role_Offset, DbcFile::Role_Min,
        DbcFile::Role_Max, DbcFile::Role_ByteOrder, DbcFile::Role_ValueType,
        DbcFile::Role_Receivers
    };
    ...
    const std::vector<int> ecuRoles = {
        DbcFile::Role_EcuTotalSignals
    };
...
TEST_F(DbcModelTest, Qt_ModelConsistency_Check) {
    ...
    QAbstractItemModelTester tester(model.get(), QAbstractItemModelTester::FailureReportingMode::Fatal);
}
...
TEST_F(DbcModelTest, Hierarchy_BuildsCorrectly) {
    ...
    EXPECT_EQ(getVal(overviewIdx, DbcFile::DbcRoles::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Overview);

    // Check ECU (Row 1)
    QModelIndex ecuIdx = model->index(1, 0, QModelIndex());
    EXPECT_EQ(getVal(ecuIdx).toString().toStdString(), "TestNode");
    EXPECT_EQ(getVal(ecuIdx, DbcFile::DbcRoles::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Ecu);
    ...
    EXPECT_EQ(getVal(msgIdx, DbcFile::DbcRoles::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Message);
    ...
    EXPECT_EQ(getVal(sigIdx, DbcFile::DbcRoles::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Signal);
}
...
TEST_F(DbcModelTest, OverviewDataIsCorrect) {
    ...
    EXPECT_EQ(getVal(idx, DbcFile::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Overview);
    EXPECT_EQ(getVal(idx, DbcFile::Role_ChildCount).toInt(), 0); // Overview has no children in tree logic (it's a leaf in root)

    // 2. Check DisplayRole for ALL Columns (0 to 5)
    // Col 0: Filename
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::OvFilename), Qt::DisplayRole).toString(), "simple.dbc");
    // Col 1: Version
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::OvVersion), Qt::DisplayRole).toString(), "1.0");
    // Col 2: Node Count
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::OvEcuCount), Qt::DisplayRole).toInt(), 1);
    // Col 3: Msg Count
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::OvMsgCount), Qt::DisplayRole).toInt(), 1);
    // Col 4: Sig Count
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::OvSigCount), Qt::DisplayRole).toInt(), 1);
    // Col 5: Orphans
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::OvOrphans), Qt::DisplayRole).toInt(), 0);
...
TEST_F(DbcModelTest, EcuDataIsCorrect) {
    ...
    // 1. Check Custom Roles
    EXPECT_EQ(getVal(idx, DbcFile::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Ecu);
    ...
    // Col 0: Name
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::EcuName), Qt::DisplayRole).toString(), "MotorController");

    // Col 1: Total Signals (Data comes from item->data(1))
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::EcuTotalSignals), Qt::DisplayRole).toInt(), 3);
...
TEST_F(DbcModelTest, MessageDataIsCorrect) {
    ...
    // 1. Check Custom Roles
    EXPECT_EQ(getVal(idx, DbcFile::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Message);
    ...
    // 2. Check DisplayRole for ALL Columns
    // Col 0: Name
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::MsgName), Qt::DisplayRole).toString(), "TestMessage");
    // Col 1: ID
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::MsgId), Qt::DisplayRole).toUInt(), 0x123);
    // Col 2: DLC
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::MsgDlc), Qt::DisplayRole).toInt(), 8);
    // Col 3: Sender
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::MsgSender), Qt::DisplayRole).toString(), "TestNode");
    // Col 4: SigCount (Calculated)
    EXPECT_EQ(model->data(idx.siblingAtColumn(DbcFile::Constants::Columns::MsgSigCount), Qt::DisplayRole).toInt(), 1);
...
TEST_F(DbcModelTest, SignalDataIsCorrect) {
    ...
    // 1. Check Custom Roles
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_ItemType).value<Core::DbcItemType>(), Core::DbcItemType::Signal);
    ...
    // Col 0: Name
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigName), Qt::DisplayRole).toString(), "UnsignedLittleEndSignal");

    // Col 1: Unit
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigUnit), Qt::DisplayRole).toString(), "km/h");
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_Unit).toString(), "km/h");

    // Col 2: StartBit
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigStartBit), Qt::DisplayRole).toInt(), 7);
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_StartBit).toInt(), 7);

    // Col 3: Length
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigLength), Qt::DisplayRole).toInt(), 16);
    ...
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigByteOrder), Qt::DisplayRole).toString().toStdString(), endianStr);
    ...
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigValueType), Qt::DisplayRole).toString().toStdString(), typeStr);
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_ValueType).toString().toStdString(), typeStr);

    // Col 6: Factor (Formatted String "0.5")
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigFactor), Qt::DisplayRole).toString(), "0.5");
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_Factor).toString(), "0.5");

    // Col 7: Offset (Formatted String "-10")
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigOffset), Qt::DisplayRole).toString(), "-10");
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_Offset).toString(), "-10");

    // Col 8: Min
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigMin), Qt::DisplayRole).toDouble(), -100.0);
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_Min).toDouble(), -100.0);

    // Col 9: Max
    EXPECT_EQ(model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigMax), Qt::DisplayRole).toDouble(), 100.0);
    EXPECT_EQ(getVal(mainTestSig , DbcFile::Role_Max).toDouble(), 100.0);

    // Col 10: Receivers
    QString recStr = model->data(mainTestSig .siblingAtColumn(DbcFile::Constants::Columns::SigReceivers), Qt::DisplayRole).toString();
    ...
    EXPECT_EQ(model->data(signedBigEndSig .siblingAtColumn(DbcFile::Constants::Columns::SigByteOrder), Qt::DisplayRole).toString().toStdString(), endianStr);
    ...
    EXPECT_EQ(model->data(signedBigEndSig.siblingAtColumn(DbcFile::Constants::Columns::SigValueType), Qt::DisplayRole).toString().toStdString(), typeStr);
...
TEST_F(DbcModelTest, HandlesEmptyConfig)
{
    ...
    EXPECT_EQ(getVal(overview,
        DbcFile::DbcRoles::Role_ItemType).value<Core::DbcItemType>(),
...
TEST_F(DbcModelTest, EdgeCase_OrphanHandling) {
    ...
    for(int i=0; i < model->rowCount(QModelIndex()); ++i) {
        ...
        if(getVal(idx, DbcFile::Role_ItemType).value<Core::DbcItemType>() == Core::DbcItemType::OrphanHolder) {
            orphanHolderIdx = idx;
            break;
        }
    }
