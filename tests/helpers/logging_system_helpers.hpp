...
inline void acceptDialogAsDbc()
{
    QTimer::singleShot(300, []() {
        ...
        for (auto* btn : d->findChildren<QPushButton*>())
        {
            if (btn->text().trimmed() == "Start" && btn->isEnabled())
            {
                btn->click();
                return;
            }
...
{
    for (const QDir logsDir("logs");
         const auto& fi : logsDir.entryInfoList({"session_*.mf4"}, QDir::Files, QDir::Time))
    {
