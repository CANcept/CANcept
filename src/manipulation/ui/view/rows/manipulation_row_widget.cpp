...
#include "manipulation_row_widget.hpp"
...
ManipulationRowWidget::ManipulationRowWidget(Providers providers, const bool showRemove,
                                             QWidget* parent)
    : QWidget(parent), m_providers(std::move(providers))
{
    ...
    connect(m_typeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &ManipulationRowWidget::onTypeChanged);
    ...
    connect(m_removeButton, &QPushButton::clicked, this, &ManipulationRowWidget::removeRequested);
...
auto ManipulationRowWidget::currentEntry() const -> SectionEntry
{
    SectionEntry entry;
...
void ManipulationRowWidget::setFromEntry(const SectionEntry& entry)
{
...
void ManipulationRowWidget::connectOptionsSignals() const
{
    if (!m_currentOptions)
    {
        return;
...
auto ManipulationRowWidget::event(QEvent* event) -> bool
{
