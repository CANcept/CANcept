...
struct DBCParsedEvent final : Event {
    ...
    DBCParsedEvent(DbcConfig config, std::string filePath) : config(std::move(config)), filePath(std::move(filePath)) {};
};
...
struct DBCParseErrorEvent final : Event {
    ...
    DBCParseErrorEvent(std::string errorMessage, std::string filePath) : errorMessage(std::move(errorMessage)), filePath(std::move(filePath)) {};
};
