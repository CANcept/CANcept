...
/**
 * @class FaultInjectorDialogModel
 * @brief Single source of truth for the fault dialog state.
 */
class FaultInjectorDialogModel : public QObject
{
    Q_OBJECT

   public:
    explicit FaultInjectorDialogModel(QObject* parent);
    ~FaultInjectorDialogModel() override = default;
    ...
    /**
     * @brief Converts the stored entries to a typed Fault and resets the model.
     * @return The built Fault, or std::nullopt if any entry is invalid.
     */
    auto acquire() -> std::optional<Fault>;
