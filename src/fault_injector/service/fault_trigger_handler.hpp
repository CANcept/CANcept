 ...
 */
inline auto firesRawTrigger(const RawTrigger& trigger, const Core::RawCanMessage& message,
                            std::mt19937& random) -> bool
{
    return std::visit(entt::overloaded{
                          // lambda to check if the message has a certain id
                          [&](const IDTrigger& t) { return message.messageId == t.id; },

                          // lambda to check if the message has a certain dlc
                          [&](const DLCTrigger& t) { return message.dlc == t.dlc; },
